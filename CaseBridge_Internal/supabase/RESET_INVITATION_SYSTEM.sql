...
BEGIN
    ...
    INSERT INTO public.invitations (email, role_preassigned, firm_id, first_name, last_name, token, status)
    VALUES (p_email, p_role, p_firm_id, p_first_name, p_last_name, v_token, 'pending')
...
BEGIN
    SELECT * INTO v_invite FROM public.invitations WHERE token = p_token AND status = 'pending';
    IF NOT FOUND THEN RETURN 'ERROR: Invalid or expired token'; END IF;
