...
export default function CaseDetail() {
    ...
    // 6. Fetch Document Vault
    const { data: vaultDocuments = [] } = useQuery({
     ...
     if (loading) {
         return (
             <div className="min-h-[60vh] flex flex-col items-center justify-center gap-4">
                 ...
                 <p className="text-muted-foreground font-bold tracking-widest uppercase text-xs animate-pulse">Accessing Secure File...</p>
             </div>
         );
     }
    ...
    if (isError) {
        return (
            <div className="max-w-xl mx-auto py-20 animate-fade-in relative z-10 px-4">
                <div className="bg-card/50 border border-dashed border-red-500/30 rounded-[2rem] flex flex-col items-center justify-center py-16 px-6 text-center shadow-lg shadow-red-500/5">
                    <div className="w-16 h-16 rounded-full bg-red-500/10 border border-red-500/20 flex items-center justify-center mb-6 shadow-sm">
                        <Shield size={28} className="text-red-500/50" />
                    </div>
                    <h2 className="text-2xl font-black mb-3 text-foreground">Connection Error</h2>
                    <p className="text-muted-foreground mb-8 text-sm leading-relaxed">
                        We're having trouble connecting to the secure server. This might be due to an unstable internet connection or a temporary server outage.
                    </p>
                    <button 
                        onClick={() => refetch()}
                        className="px-10 py-3.5 bg-foreground text-background font-black rounded-[var(--radius-neumorph)] uppercase tracking-widest text-[10px] shadow-xl hover:scale-105 active:scale-95 transition-all flex items-center gap-3"
                    >
                        <Loader2 size={16} className="animate-spin" /> Retry Connection
                    </button>
                    <Link to="/cases" className="mt-6 text-xs font-bold text-muted-foreground hover:text-foreground uppercase tracking-widest transition-colors">
                        Return to Case List
                    </Link>
                </div>
            </div>
    ...
    return (
        <div className="animate-fade-in relative max-w-5xl mx-auto pb-10">
            ...
            <div className="relative z-10 px-2 sm:px-0">
                ...
                {/* Tab Navigation */}
                    <div className="flex bg-input/50 p-1 rounded-2xl border border-border shadow-inner">
                        <button
                            onClick={() => setActiveTab('timeline')}
                            className={`px-4 sm:px-6 py-2.5 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'timeline'
                        ...
                        <button
                            ...
                            className={`px-4 sm:px-6 py-2.5 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all relative ${activeTab === 'messages'
                                ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/20'
                                : 'text-muted-foreground hover:text-foreground hover:bg-input'
                                }`}
                        >
                            Messages
                            {(unreadCount || 0) > 0 && (
                                <span className="absolute -top-1 -right-1 w-4 h-4 bg-red-500 text-white text-[8px] flex items-center justify-center rounded-full animate-bounce shadow-lg ring-2 ring-background">
                                    {unreadCount}
                                </span>
                            )}
                        </button>
                        <button
                            ...
                            className={`px-4 sm:px-6 py-2.5 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'vault'
                                ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/20'
                                : 'text-muted-foreground hover:text-foreground hover:bg-input'
                                }`}
                        >
                        ...
                        <button
                            ...
                            className={`px-4 sm:px-6 py-2.5 rounded-xl text-[10px] sm:text-xs font-black uppercase tracking-widest transition-all ${activeTab === 'audit'
                                ? 'bg-primary text-primary-foreground shadow-lg shadow-primary/20'
                                : 'text-muted-foreground hover:text-foreground hover:bg-input'
                                }`}
                        >
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 sm:gap-10">
                    ...
                    <div className="lg:col-span-2 space-y-8 sm:space-y-10">
                        ...
                        {activeTab === 'vault' && (
                            <section className="animate-in slide-in-from-right-4 duration-500">
                                <div className="bg-card border border-border shadow-sm rounded-[2rem] p-6 sm:p-10">
                                    <div className="flex items-center justify-between mb-8">
                                        <div>
                                            <h2 className="text-xl sm:text-2xl font-black text-foreground mb-1">Document Vault</h2>
                                            <p className="text-xs text-muted-foreground font-bold uppercase tracking-widest">Case-Wide Secure Storage</p>
                                        </div>
                                        <Lock size={24} className="text-primary/40" />
                                    </div>

                                    {(vaultDocuments || []).length > 0 ? (
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            {vaultDocuments.map((doc: any, idx: number) => (
                                                <div key={idx} className="p-5 bg-input/50 border border-border hover:border-primary/40 rounded-[2rem] transition-all group/doc shadow-sm">
                                                    <div className="flex items-start gap-4">
                                                        <div className="w-12 h-12 bg-primary/10 rounded-2xl flex items-center justify-center text-primary group-hover/doc:scale-110 transition-transform shadow-sm border border-primary/20">
                                                            <FileText size={20} />
                                                        </div>
                                                        <div className="flex-1 min-w-0">
                                                            <h4 className="text-sm font-bold text-foreground truncate">{doc.document.filename}</h4>
                                                            <div className="flex items-center gap-2 mt-1">
                                                                <span className="text-[9px] text-primary/70 font-black uppercase tracking-widest">{doc.source}</span>
                                                                <span className="text-[9px] text-muted-foreground">•</span>
                                                                <span className="text-[9px] text-muted-foreground">{new Date(doc.document.uploaded_at).toLocaleDateString()}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div className="mt-6 flex items-center gap-2">
                                                        <button 
                                                            onClick={() => handleViewDocument(doc.document.file_url, doc.document.filename)}
                                                            className="flex-1 py-2.5 bg-input border border-border hover:border-primary/50 text-foreground rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-sm"
                                                        >
                                                            <Eye size={12} /> Preview
                                                        </button>
                                                        <button 
                                                            onClick={() => handleDownload(doc.document.file_url)}
                                                            className="flex-1 py-2.5 bg-primary/10 border border-primary/20 hover:bg-primary/20 text-primary rounded-xl text-[10px] font-black uppercase tracking-widest transition-all flex items-center justify-center gap-2 shadow-sm"
                                                        >
                                        ...
                                        <div className="py-20 text-center border-2 border-dashed border-border rounded-[2rem] bg-input/20">
                                            <Shield size={48} className="mx-auto text-muted-foreground/20 mb-4" />
                                            <p className="text-sm text-muted-foreground font-black uppercase tracking-widest">No Documents Shared Yet</p>
                                        </div>
                                    )}
                                </div>
                            </section>
                        ...
                        {activeTab === 'audit' && (
                            <section className="animate-in slide-in-from-right-4 duration-500">
                                <div className="bg-card border border-border shadow-sm rounded-[2rem] p-6 sm:p-10">
                                    <div className="flex items-center justify-between mb-8">
                                        <div>
                                            <h2 className="text-xl sm:text-2xl font-black text-foreground mb-1">Case History</h2>
                                            <p className="text-xs text-muted-foreground font-bold uppercase tracking-widest">Real-Time Interaction Stream</p>
                                        </div>
                                        <Clock size={24} className="text-primary/40" />
                                    </div>

                                    {(interactionHistory || []).length > 0 ? (
                                        <div className="space-y-6 relative before:absolute before:inset-0 before:left-5 before:w-0.5 before:bg-border/50 before:h-full pb-2">
                                            {interactionHistory.map((item: any, idx: number) => (
                                                <div key={idx} className="relative pl-12 group/history">
                                                    <div className={`absolute left-0 top-0 w-10 h-10 rounded-full border-4 border-card flex items-center justify-center z-10 shadow-sm ${
                                                        item.type === 'update' ? 'bg-emerald-500 text-white' :
                                                        item.type.includes('signature') ? 'bg-primary text-primary-foreground' :
                                                        'bg-input text-muted-foreground'
                                                    }`}>
                                                        {item.type === 'update' ? <FileText size={14} /> :
                                                         item.type.includes('signature') ? <PenTool size={14} /> :
                                                         <Clock size={14} />}
                                                    </div>
                                                    <div className="bg-input/30 border border-border p-4 rounded-2xl group-hover/history:bg-input/50 transition-colors">
                                                        <div className="flex items-center justify-between mb-1">
                                                            <p className="text-[10px] text-muted-foreground font-black uppercase tracking-widest">
                                                                {new Date(item.created_at).toLocaleString()}
                                                            </p>
                                                            <span className="text-[8px] font-black uppercase tracking-tighter bg-card px-2 py-0.5 rounded-full border border-border">
                                                                {item.type.replace('_', ' ')}
                                                            </span>
                                                        </div>
                                        ...
                                        <div className="py-20 text-center border-2 border-dashed border-border rounded-[2rem] bg-input/20">
                                            ...
                                            <p className="text-sm text-muted-foreground font-black uppercase tracking-widest">Tracking Your Journey...</p>
                                        </div>
                                    )}
                                </div>
                            </section>
            ...
            <ReviewModal
                ...
                matterId={matter?.id}
                onSuccess={() => queryClient.invalidateQueries({ queryKey: ['matter_review', id] })}
            />
        </div>
    );
}
