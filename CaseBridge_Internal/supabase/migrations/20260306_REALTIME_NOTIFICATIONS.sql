...
-- Update the process_event_notifications function to handle client signups
...
DECLARE
    ...
    v_client_name TEXT;
BEGIN
    -- [NEW] Event: Client Signup
    -- Notify all firm admins about the new client registration
    IF NEW.action = 'client_signup' THEN
        -- Get the client's name from the target (client_profiles)
        SELECT full_name INTO v_client_name 
        FROM public.client_profiles 
        WHERE id = NEW.target_id;
        
        IF v_client_name IS NULL THEN
             v_client_name := 'A new client';
        END IF;
        ...
        FOR v_admin_cm IN 
            ...
            WHERE status = 'active'
            AND LOWER(role) IN ('admin', 'admin_manager')
        LOOP
            ...
            VALUES (v_admin_cm.user_id, NEW.action, 'in_app', jsonb_build_object(
                ...
                'message', v_client_name || ' has successfully registered on the Client Portal.',
                'link', '/internal/settings/staff' -- or a clients directory
            ));
        END LOOP;
