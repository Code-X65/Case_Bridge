-- ==========================================
-- MASTER GOVERNANCE & LIVE JOURNEY SYSTEM
-- ==========================================

-- 1. STABLE HELPER FUNCTIONS
-- Required for firm isolation and non-recursive RLS
...
RETURNS BOOLEAN LANGUAGE sql SECURITY DEFINER SET search_path = public AS $$
    SELECT EXISTS (SELECT 1 FROM public.user_firm_roles WHERE user_id = auth.uid() AND firm_id = p_firm_id AND (role = 'admin_manager' OR role = 'admin'));
$$;
...
-- 3. LIVE INVITATION RPCS
-- These are the "live" versions of the mock functions
...
BEGIN
    ...
    INSERT INTO public.audit_logs (actor_id, firm_id, action, metadata) VALUES (auth.uid(), p_firm_id, 'staff_invited', jsonb_build_object('email', p_email, 'role', p_role));
    RETURN v_token;
END;
...
BEGIN
    ...
    INSERT INTO public.audit_logs (actor_id, firm_id, action, metadata) VALUES (auth.uid(), v_invite.firm_id, 'staff_invite_resent', jsonb_build_object('email', v_invite.email));
    RETURN v_new_token;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- 4. ACCEPTANCE LOGIC (SAFE TOKEN LOOKUP)
...
-- 5. AUTO-BIND TRIGGER (The Onboarding Magic)
CREATE OR REPLACE FUNCTION public.handle_invited_user_signup() RETURNS TRIGGER AS $$
...
CREATE TRIGGER on_invited_auth_user_created AFTER INSERT ON auth.users FOR EACH ROW EXECUTE FUNCTION public.handle_invited_user_signup();

SELECT '✅ Staff Invite system fully operational and LIVE.' AS status;
