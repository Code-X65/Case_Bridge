...
export default function MessagesPage() {
    ...
    // Fetch Matters (Cases with messaging enabled)
    useEffect(() => {
        const fetchMatters = async () => {
            ...
            if (!error && data) {
                setMatters(data);
                if (matterIdFromUrl) {
                    const match = data.find(m => m.id === matterIdFromUrl);
                    if (match) setSelectedMatter(match);
                } else if (data.length > 0 && !selectedMatter) {
    ...
    useEffect(() => {
        ...
        const channel = supabase
            .channel(`matter_${selectedMatter.id}`)
            .on('postgres_changes', {
                event: 'INSERT',
                schema: 'public',
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!newMessage.trim() || !selectedMatter || !user) return;
        ...
        if (!error) {
            ...
            await supabase.rpc('log_client_event', {
                p_action: 'client_sent_message',
                p_details: { matter_id: selectedMatter.id }
            });
        }
        setSending(false);
    };
                        ...
                        <div className="p-4 sm:p-6 bg-slate-900/60 border-t border-white/5">
                            <form onSubmit={handleSendMessage} className="relative">
                                ...
                                <div className="absolute right-1.5 top-1/2 -translate-y-1/2 flex items-center gap-1">
                                    ...
                                    >
                                        {sending ? <span className="animate-spin block w-4 h-4 sm:w-5 sm:h-5 border-2 border-white/20 border-t-white rounded-full"></span> : <Send size={18} />}
                                    </button>
                                </div>
                            </form>
